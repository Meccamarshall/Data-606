...
A streak length of 1 means that one basket was shot by Kobe Bryant or hit before missing the next few shots. When there is a streak length of 0, that means that Kobe Bryant did not make any hits or scored consecutive misses without any hits.
...
Kobe's streak distribution is right skewed and unimodal(one peak). Typical streak length was zero and longest streak length was four.
```{r}
summary(kobe_streak)
```
...
I decided to use a probability of 0.29 and 0.8 to test out the unfair coin flips. In my example, I set seed to my birth date in order to prevent unique outcomes when knitting. Heads appeared 24 times and Tails appeared 76

```{r}
set.seed(112490)

results <- c("heads", "tails")
simulation_unfair_coin <- sample(results, size = 100, replace = TRUE, prob = c(0.29, 0.8))
